...
In this phase, we take the best hyperparameters found in Phase 2 for each architecture and evaluate how different augmentation strategies affect performance when training on the full dataset. We will test several combinations of augmentation techniques (see Table~\ref{tab:aug_strategies}):
...
Each configuration will use 3 seeds. This will help us understand which augmentation strategies help the most, and whether the two architectures prefer different augmentation approaches. The best augmentation strategy for each model will be carried forward to Phase 4.
...
For the episodic training, we will perform a 10-way classification task where the model must distinguish between some number of examples from each of the 10 classes, with queries used to evaluate performance. We will run a sufficiently large number of episodes during training to ensure stable learning.
...
As a sanity check, we will also train both architectures on the reduced dataset using standard supervised learning (not few-shot episodic training) with the best augmentation strategy and hyperparameters found in Phase 3. This lets us compare how the few-shot approach performs relative to a conventional training pipeline under the same data scarcity constraint, as summarized in Table~\ref{tab:standard_training}.
...
\midrule
Standard supervised & ResNet-18 & Reduced & 3 \\
Standard supervised & DeiT-Tiny & Reduced & 3 \\
\bottomrule
\end{tabular}
\end{table}
...
In addition to evaluating the individual performance of ResNet-18 and DeiT-Tiny, we introduce ensemble methods to investigate whether combining CNN and Vision Transformer representations leads to improved performance. For all ensemble experiments, we use the best-performing model configurations identified in Phase 2 and Phase 3.
...
The general division of responsibilities between team members is presented in Table~\ref{tab:responsibilities}.

\begin{table}[H]
\centering
\caption{General division of responsibilities between team members.}
\label{tab:responsibilities}
\begin{tabular}{@{}p{0.32\textwidth}p{0.32\textwidth}p{0.32\textwidth}@{}}
\toprule
\textbf{Area} & \textbf{Bart\l{}omiej Borycki} & \textbf{Micha\l{} Iwaniuk} \\
\midrule

Project setup
& Experiment structure, reproducibility, logging framework
& Data preparation, training environment \\

\addlinespace

Phases 1--2 (Baselines \& Hyperparameters)
& CNN-focused experiments and analysis
& Vision Transformer experiments and analysis \\

\addlinespace

Phase 3 (Augmentation Study)
& Augmentation experiments for CNN models
& Augmentation experiments for ViT models \\

\addlinespace

Phase 4 (Few-shot Learning)
& Prototypical Networks with DeiT-Tiny backbone 
& Prototypical Networks with ResNet-18 backbone \\

\addlinespace

Embedding Analysis \& Evaluation
& Quantitative clustering metrics and comparisons
& Visualization of embedding spaces and interpretation \\

\addlinespace

\midrule
Shared responsibilities
& \multicolumn{2}{p{0.64\textwidth}}{Experiment planning and final writing of the report.} \\
\bottomrule
\end{tabular}
\end{table}
\end{document}